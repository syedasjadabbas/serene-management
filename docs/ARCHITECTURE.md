...
```text
 ...
 modules/<domain>/*.repository.ts ── Prisma queries (select shapes, raw SQL for reports)
    │
 PostgreSQL 17 ── constraints, exclusion constraints, triggers = last line of defence
```
...
| Database rules        | Vitest + **PGlite** (PostgreSQL 18 in WASM, all migrations applied)                                 | check/exclusion constraints, triggers, composite-FK isolation — runs anywhere, no Docker                                                                                                                                                                                                                                                                                            | `tests/db/` (implemented, passing) |
| Integration (service) | Vitest + real PostgreSQL 17 (docker compose / CI service), migrated schema, per-test data factories | every workflow in PMS_WORKFLOWS.md: reservation create/modify/cancel/reinstate with inventory, room assignment conflicts, check-in, check-out, postings, routing, split/transfer, payments/refunds, cashier close, housekeeping transitions, night audit success and failure-rollback, business date roll, multi-property isolation, concurrency (two agents selling the last room) | `tests/integration/`               |
