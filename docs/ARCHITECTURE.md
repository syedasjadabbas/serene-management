...
- **Explicit row locks** on contention points, always taken in a fixed order to avoid deadlocks. As implemented (Phases 2–3; decision D13):
  ...
  5. `block_allocations` rows, same order (groups, later)
  6. `property_sequences` row (short hold)
  7. `rooms` rows whose occupancy changes, ordered by id (`FOR UPDATE`)
  8. `folios` rows being posted to, ordered by id (billing, later)
